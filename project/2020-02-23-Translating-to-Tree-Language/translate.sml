...
fun newLevel {parent,name,formals} =
    Lev({parent=parent, frame=Frame.newFrame{name=name,formals=true::formals}}, ref ())


(* function to return formals associated with the frame in this level, excluding the static link *)
fun formals lev =
    case lev of
      Top => nil
...
(* function to allocate a new local variable either on frame or in register *)
...
fun subscriptVar (base,offset): exp =
    Ex(memplus(unEx(base), T.BINOP(T.MUL,unEx(offset),T.CONST(Frame.wordSize))))


fun fieldVar (base,id,flist) : exp =
    let fun findindex (index,elem,list) =
            if elem = hd(list) then index
            else findindex(index+1,elem,tl(list))
	in
      Ex(memplus(unEx(base), T.BINOP(T.MUL,T.CONST(findindex(0,id,flist)), T.CONST(Frame.wordSize))))
    end
